...
7. You should now have prefect installed. Run the prefect server locally using the `prefect orion start` command to monitor flows. Follow this [video](https://www.youtube.com/watch?v=ae-CV2KfoN0&list=PL3MmuxUbc_hJed7dXYoJw8DoCuVHhGEQb) to help setup SSH in a VS code environment, which allows for port forwarding from your cloud VM to your local machine. This is needed to access the Orion UI. and In another terminal, `cd` into the `flows` directory and run the command `python deployment.py` to setup the prefect deployment. Make sure you setup the following prefect blocks before running:
