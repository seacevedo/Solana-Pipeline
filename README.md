...
7. You should now have prefect installed. Run the prefect server locally using the `prefect orion start` command to monitor flows. In another terminal, `cd` into the `flows` directory and run the command `python deployment.py` to setup the prefect deployment. Make sure you setup the following prefect blocks before running:
